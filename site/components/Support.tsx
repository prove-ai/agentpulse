const CHANNELS = [
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
        <rect x="3" y="5" width="18" height="14" rx="2.5" />
        <path d="m3.5 7 8.5 6 8.5-6" />
      </svg>
    ),
    tint: "blue" as const,
    title: "Email support",
    desc: "Get help directly from our team. No login required.",
    linkLabel: "leyla@proveai.com",
    href: "mailto:leyla@proveai.com",
    note: "Response within 2 business days",
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
        <rect x="3" y="4.5" width="18" height="16" rx="2.5" />
        <path d="M3 9.5h18M8 2.8v3.4M16 2.8v3.4" />
      </svg>
    ),
    tint: "violet" as const,
    title: "Talk to an engineer",
    desc: "Book a 30-minute walkthrough of a real drift investigation.",
    linkLabel: "Book a call",
    href: "https://calendly.com/nick-proveai/agent-pulse-demo",
    note: "30 min · video call",
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
        <path d="M12 2C6.48 2 2 6.58 2 12.25c0 4.53 2.87 8.37 6.84 9.73.5.09.68-.22.68-.49 0-.24-.01-.88-.01-1.73-2.78.62-3.37-1.37-3.37-1.37-.45-1.18-1.11-1.5-1.11-1.5-.91-.63.07-.62.07-.62 1 .07 1.53 1.06 1.53 1.06.89 1.57 2.34 1.12 2.91.85.09-.66.35-1.12.63-1.37-2.22-.26-4.56-1.14-4.56-5.07 0-1.12.39-2.03 1.03-2.75-.1-.26-.45-1.3.1-2.7 0 0 .84-.28 2.75 1.05a9.4 9.4 0 0 1 2.5-.34c.85 0 1.7.12 2.5.34 1.91-1.33 2.75-1.05 2.75-1.05.55 1.4.2 2.44.1 2.7.64.72 1.03 1.63 1.03 2.75 0 3.94-2.34 4.8-4.57 5.06.36.32.68.94.68 1.9 0 1.37-.01 2.47-.01 2.81 0 .27.18.59.69.49A10.02 10.02 0 0 0 22 12.25C22 6.58 17.52 2 12 2Z" />
      </svg>
    ),
    tint: "neutral" as const,
    title: "GitHub Discussions",
    desc: "Ask questions, report issues, or tell us how you investigate agent failures.",
    linkLabel: "Open Discussions",
    href: "https://github.com/prove-ai/agentpulse/discussions",
    note: "Public · community",
  },
];

export default function Support() {
  return (
    <section className="support support-page">
      <div className="support-inner">
        <div className="num">Support</div>
        <h2>Reach us your way</h2>
        <div className="support-cards">
          {CHANNELS.map((c) => (
            <div key={c.title} className="scard">
              <div className={`scard-icon tint-${c.tint}`}>{c.icon}</div>
              <h3>{c.title}</h3>
              <p>{c.desc}</p>
              <a className="scard-link" href={c.href}>
                {c.linkLabel}
              </a>
              <div className="scard-note">{c.note}</div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
