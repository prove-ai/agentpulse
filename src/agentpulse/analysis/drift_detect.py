...
def metric_breaches(series: dict, cfg: dict) -> list[dict]:
    ...
    for metric, base in mcfg.items():
        ...
        for entity in series.get(scope, {}):
            ...
            # Drift cannot START inside the window that defines "normal": baseline
            # runs deviating from their own mean is variance, not drift, and letting
            # them claim drift_start breaks event correlation (a v2/v3 deploy can
            # never be "near" a start that noise placed at run 0-4). Candidate for
            # keeping permanently.
            for _i in range(min(baseline_runs, len(pr))):
                pr[_i] = False
...
def classify_drift(series: dict, change_log: list[dict], cfg: dict,
    ...
    # Cluster dedup: an agent finding and a handoff finding built from the SAME
    # underlying breaches are two views of one drift (adjacency is symmetric, so
    # each corroborates the other). Surface the cluster once, preferring the
    # handoff - the edge names both components involved. Path/system findings are
    # never collapsed into component findings (different story). Candidate for
    # keeping permanently.
    def _sigset(f):
        sigs = {(f["trigger"]["scope"], f["trigger"]["entity"], f["trigger"]["metric"])}
        sigs |= {(s["scope"], s["entity"], s["metric"]) for s in f["supporting"]}
        return sigs

    kept = []
    for f in sorted(findings, key=lambda f: (-_SEV_RANK[f["severity"]],
                                             0 if f["scope"] == "handoffs" else 1,
                                             f["drift_start"])):
        fs = _sigset(f)
        dup = any(
            f["scope"] in ("agents", "handoffs") and k["scope"] in ("agents", "handoffs")
            and (fs <= _sigset(k)
                 or len(fs & _sigset(k)) / max(1, len(fs | _sigset(k))) >= 0.6)
            for k in kept
        )
        if not dup:
            kept.append(f)

    kept.sort(key=lambda f: (-_SEV_RANK[f["severity"]], f["drift_start"]))
    return kept
...
def investigate(series: dict, change_log: list[dict], cfg: dict,
    ...
    """
    from agentpulse.analysis.drift_chains import build_topology, edges_from_handoff_series, detect_chains
