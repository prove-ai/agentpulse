"""Load and apply the handoff-drift rules from drift_rules.yaml.
...
Resolution order:
  1. <AGENTPULSE_HOME>/config/drift_rules.yaml  (user override, ~/.agentpulse)
  2. the default drift_rules.yaml shipped inside the package
"""
...
from agentpulse.paths import config_dir

_DEFAULT_CONFIG_PATH = Path(__file__).parent.parent / "config" / "drift_rules.yaml"


def _config_path() -> Path:
    user_path = config_dir() / "drift_rules.yaml"
