...
# ANTHROPIC_API_KEY for the Opus call in get_next_check_steps — MCP hosts
# (Claude Code CLI or Desktop) spawn the server without a shell env.
load_env_file()

# The `mcp` package is an optional extra (pip install agentpulse[mcp]). The
# drift CLI imports this module for its functions, so degrade gracefully: when
# FastMCP is absent, tools stay plain callables and only `agentpulse mcp` fails.
try:
    ...
    class FastMCP:  # type: ignore[no-redef]
        ...
        def run(self) -> None:
            raise SystemExit(
                ...
                "install with: pip install agentpulse[mcp]")

from agentpulse.storage.sqlite_store import (
    list_available_dbs, resolve_db_path, set_active_db_path, get_baseline_version,
)
from agentpulse.analysis.layer1_raw import list_runs
from agentpulse.analysis.version_drift import compare_versions
from agentpulse.analysis.diagnose import suggest_next_checks
from agentpulse.reporter.dashboard import investigations_for
...
def _compare_pair(all_runs, labels, vof, base_v, target_v, task_type, ver_options,
    ...
    from agentpulse.reporter.dashboard import _metrics_for_run
    ...
    if len(base_runs) < _MIN_RUNS or len(tgt_runs) < _MIN_RUNS:
        return {"ok": False, "comparing": comparing,
                "reason": (f"Need >={_MIN_RUNS} runs per version (have {len(base_runs)} in v{base_v}, "
...
def get_version_comparison(project: str | None = None, base_version: int | None = None,
    ...
    """
    from agentpulse.reporter.dashboard import _ver_of_factory
    from agentpulse.storage.sqlite_store import get_versions
