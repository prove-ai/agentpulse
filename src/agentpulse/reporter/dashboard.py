...
from agentpulse.paths import load_env_file

load_env_file()  # e.g. ANTHROPIC_API_KEY from <AGENTPULSE_HOME>/.env
...
from agentpulse.analysis.version_drift import compare_versions, version_insights
from agentpulse.analysis.dag import detect_parallel_groups, critical_path
from agentpulse.analysis.trends import build_trends
from agentpulse.analysis.changes import potentially_related_changes, diff_configs, _parse_cfg
from agentpulse.storage.sqlite_store import (
...
def _collect_handoff_occurrences(runs: list[dict]) -> list[dict]:
    ...
    """
    from agentpulse.analysis.run_metrics import is_clean_termination
...
def _evaluate_rules(early: dict, recent: dict, freq_delta_pct: float, cfg: dict) -> list[str]:
    ...
    from agentpulse.analysis.drift_config import evaluate
...
def compute_handoff_drift(early_runs: list[dict], recent_runs: list[dict]) -> dict:
    ...
    """
    from collections import defaultdict
    from agentpulse.analysis.drift_config import load_drift_config
    ...
    for pair in all_pairs:
        ...
        from agentpulse.analysis.drift_config import priority_order
...
def home():
    ...
    import datetime as _dt
    from agentpulse.storage.sqlite_store import resolve_db_path, set_active_db_path
    projects, activity = [], []
...
def explore():
    ...
    from agentpulse.analysis.metric_series import per_run_series, control_band, metric_impact, metrics_of
    ...
    if category == "path" and entity == "Routes":
        from collections import Counter
        from agentpulse.analysis.changes import route_key
...
def changelog():
    ...
    events = get_events()
    if not events:
        from agentpulse.storage.sqlite_store import get_prompts
...
def _attach_routes(runs):
    ...
    from agentpulse.analysis.layer1_raw import get_spans_for_runs
    from agentpulse.analysis.changes import route_topology
...
def _build_investigations(runs, sort="sev_desc", cfg_override=None):
    ...
    from agentpulse.analysis.metric_series import per_run_series
    _attach_routes(runs)
    from agentpulse.analysis.changes import build_change_log
    from agentpulse.analysis.drift_detect import investigate
    from agentpulse.analysis.drift_config import load_drift_config
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="agentpulse dashboard",
                                 description="Web dashboard for captured runs.")
    ap.add_argument("--port", type=int, default=None,
                    help="port to listen on (default: $PORT or 5001)")
    ap.add_argument("--host", default=None,
                    help="host to bind (default: 0.0.0.0 when $PORT is set, else 127.0.0.1)")
